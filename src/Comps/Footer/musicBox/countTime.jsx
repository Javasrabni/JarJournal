...
export default function CountTime({ BFPage }) {
    ...
    const play = <svg xmlns="http://www.w3.org/2000/svg" fill={BFPage ? 'black' : 'white'} viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className={`size-${BFPage ? "4" : "3"}`}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 0 1 0 1.972l-11.54 6.347a1.125 1.125 0 0 1-1.667-.986V5.653Z" />
    </svg>

    const pause = <svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24" strokeWidth={4} stroke="currentColor" className={`size-${BFPage ? "4" : "3.5"}`}>
    ...
    const musicIcon = <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className={`size-${BFPage ? '4' : "3"}`}>
                    ...
                    <div className="w-full flex flex-col gap-[8px] items-center justify-center h-[20px] p-[32px] " style={{ position: "fixed", bottom: "0px", width: "360px", backdropFilter: "blur(3px)", backgroundColor: "#00000050", borderTop: "rgb(38, 38, 38) solid 1px" }}>
                        <div className="w-full flex flex-row items-center justify-between">
                            ...
                            <div className="flex flew-row gap-[16px] items-center">
                                ...
                                {/* Pause music */}
                                <button onClick={toggleMusic} className={`w-[35px] h-[35px] p-[6px] bg-${stateMusic ? "white" : "[#ffffff50]"} text-black rounded-[50px] flex items-center justify-center`}>
                                    {stateMusic ? (
                                        <div className="">
                                            {musicIcon}
                                        </div>
                                ...
                                }} className="w-[35px] h-[35px] p-[6px] bg-white text-black rounded-[50px] ">
                                    {timeRunning ? <span className="flex items-center justify-center">{pause}</span> : <span className="flex items-center justify-center ml-[2px]">{play}</span>}
                                </button>
                                {/* next */}
